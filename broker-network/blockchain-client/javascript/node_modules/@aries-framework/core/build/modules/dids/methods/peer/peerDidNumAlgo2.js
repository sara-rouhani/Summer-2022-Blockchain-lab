...
function didToNumAlgo2DidDocument(did) {
    ...
    for (const entry of entries) {
        ...
        // Otherwise we can be sure it is a key
            ...
            const key = domain_1.Key.fromFingerprint(entryContent);
            const { getVerificationMethods } = key_type_1.getKeyDidMappingByKeyType(key.keyType);
...
function didDocumentToNumAlgo2Did(didDocument) {
    ...
    for (const [purpose, entries] of Object.entries(purposeMapping)) {
        ...
        const encoded = dereferenced.map((entry) => {
            const { getKeyFromVerificationMethod } = key_type_1.getKeyDidMappingByVerificationMethod(entry);
