...
class DidCommMessageRecord extends BaseRecord_1.BaseRecord {
    ...
    getTags() {
        ...
        const { protocolName, protocolMajorVersion, protocolMinorVersion, messageName } = messageType_1.parseMessageType(messageType);
        const thread = this.message['~thread'];
        let threadId = messageId;
        if (type_1.isJsonObject(thread) && typeof thread.thid === 'string') {
    ...
    getMessageInstance(messageClass) {
        const messageType = messageType_1.parseMessageType(this.message['@type']);
        if (!messageType_1.canHandleMessageType(messageClass, messageType)) {
