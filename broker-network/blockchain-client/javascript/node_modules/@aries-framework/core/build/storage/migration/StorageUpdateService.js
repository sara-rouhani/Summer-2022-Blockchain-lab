...
let StorageUpdateService = StorageUpdateService_1 = class StorageUpdateService {
    constructor(agentConfig, storageVersionRepository) {
    ...
    async isUpToDate() {
        ...
        const isUpToDate = updates_1.CURRENT_FRAMEWORK_STORAGE_VERSION === currentStorageVersion;
        return isUpToDate;
    }
    ...
    async setCurrentStorageVersion(storageVersion) {
        this.logger.debug(`Setting current agent storage version to ${storageVersion}`);
        const storageVersionRecord = await this.storageVersionRepository.findById(StorageUpdateService_1.STORAGE_VERSION_RECORD_ID);
        if (!storageVersionRecord) {
            this.logger.trace('Storage upgrade record does not exist yet. Creating.');
            await this.storageVersionRepository.save(new StorageVersionRecord_1.StorageVersionRecord({
     ...
     */
    async getStorageVersionRecord() {
        let storageVersionRecord = await this.storageVersionRepository.findById(StorageUpdateService_1.STORAGE_VERSION_RECORD_ID);
        if (!storageVersionRecord) {
...
StorageUpdateService = StorageUpdateService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [AgentConfig_1.AgentConfig, StorageVersionRepository_1.StorageVersionRepository])
