...
let MessageReceiver = class MessageReceiver {
    constructor(config, envelopeService, transportService, messageSender, connectionsModule, dispatcher) {
     ...
     */
    async receiveMessage(inboundMessage, { session, connection }) {
    ...
    async receivePlaintextMessage(plaintextMessage, connection) {
        const message = await this.transformAndValidate(plaintextMessage);
        const messageContext = new InboundMessageContext_1.InboundMessageContext(message, { connection });
    ...
    isEncryptedMessage(message) {
        ...
        return JWE_1.isValidJweStructure(message);
    }
    ...
    async findConnectionByMessageKeys({ recipientKey, senderKey, }) {
    ...
    async sendProblemReportMessage(message, connection, plaintextMessage) {
        const messageType = messageType_1.parseMessageType(plaintextMessage['@type']);
        if (messageType_1.canHandleMessageType(problem_reports_1.ProblemReportMessage, messageType)) {
